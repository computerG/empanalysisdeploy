...
def load_model():
    ...
    global rating_encoder
    rating_encoder = joblib.load(ENCODER_PATH)
    logging.info("Model and encoder loaded successfully.")
    print(rating_encoder)
    return model
def predict_model(model, X):
    prediction = model.predict(X)
    
    return rating_encoder.inverse_transform(prediction)
